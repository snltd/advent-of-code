...
class Aoc201907
  def solve01(input)
    input = input.as_intcode
    (0..4).to_a.permutation.map do |p|
      machines = Intcode::MachineChain.new(5)
      machines.run(input, p)
    end.max
  end

  def solve02(input)
    input = input.as_intcode
    (5..9).to_a.permutation.map do |p|
      machines = Intcode::MachineChain.new(5)
      machines.feedback_loop(input, p)
    end.max
  end
end
...
def machine_chain2(prog, inputs)
  machine_a = Intcode::Computer.new
  machine_b = Intcode::Computer.new
  machine_c = Intcode::Computer.new
  machine_d = Intcode::Computer.new
  machine_e = Intcode::Computer.new
